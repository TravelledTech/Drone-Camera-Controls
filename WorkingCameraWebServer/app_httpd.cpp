...
// ===== CUSTOM GPIO CONTROL HANDLERS =====
#include "Arduino.h"  // make sure Arduino functions are available

static esp_err_t on_handler(httpd_req_t *req) {
    digitalWrite(14, HIGH);   // was 13
    httpd_resp_send(req, "Net Armed (ON)", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}

static esp_err_t off_handler(httpd_req_t *req) {
    digitalWrite(14, LOW);    // was 13
    httpd_resp_send(req, "Net Unarmed (OFF)", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}

// Deply and Release (Switch timing later)
static esp_err_t deploy_handler(httpd_req_t *req) { // GPIO 15
  digitalWrite(15, HIGH);
  delay(200);
  digitalWrite(15, LOW);
  httpd_resp_send(req, "Deploy net", HTTPD_RESP_USE_STRLEN);
  return ESP_OK;
}

static esp_err_t release_handler(httpd_req_t *req) { // GPIO 4
  digitalWrite(12, HIGH);
  delay(200);
  digitalWrite(12, LOW);
  httpd_resp_send(req, "Release net", HTTPD_RESP_USE_STRLEN);
  return ESP_OK;
}
...
void startCameraServer() {
  ...
  httpd_register_uri_handler(camera_httpd, &on_uri);
  httpd_register_uri_handler(camera_httpd, &off_uri);
  httpd_register_uri_handler(camera_httpd, &deploy_uri);
  httpd_register_uri_handler(camera_httpd, &release_uri);
}
